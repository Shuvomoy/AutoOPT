...
Resolve `<chatgpt-pro-session-skill-dir>` to the directory containing this loaded `SKILL.md`. Independently resolve `<chatgpt-pro-handoff-skill-dir>` to the directory containing the loaded `chatgpt-pro-handoff/SKILL.md`. Substitute those absolute directories in the helper commands below, and preserve the user's current working directory so workspace-relative and artifact-relative operands keep their original meaning. Do not infer either directory from a fixed installation root or from the other skill's location.
...
```bash
python3 "<chatgpt-pro-session-skill-dir>/scripts/session_state.py" init \
  --workspace "$PWD" \
...
```bash
python3 "<chatgpt-pro-session-skill-dir>/scripts/session_state.py" claim \
  --workspace "$PWD" \
  --chat-url "https://chatgpt.com/c/..." \
...
```bash
python3 "<chatgpt-pro-session-skill-dir>/scripts/session_state.py" new-turn \
  --handoff-dir .chatgpt_handoffs/<timestamp> \
  --kind followup \
...
```bash
python3 "<chatgpt-pro-session-skill-dir>/scripts/session_state.py" complete-turn \
  --handoff-dir .chatgpt_handoffs/<timestamp> \
  --turn-id 001 \
